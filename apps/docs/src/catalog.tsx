...
export function ComponentPreview({ name }: { readonly name: string }) {
  switch (name) {
    case "Button": return <div className="docs-preview-row">{(["primary", "secondary", "ghost"] as const).flatMap((variant) => (["neutral", "accent", "danger"] as const).map((tone) => <Components.Button key={`${variant}-${tone}`} variant={variant} tone={tone}>{tone}</Components.Button>))}</div>;
    ...
    case "TextField": return <Components.TextField aria-label="Search components" defaultValue="Search components" />;
    case "TextArea": return <Components.TextArea aria-label="Release notes" defaultValue="Improved component clarity and visual hierarchy." />;
    case "Select": return <Components.Select.Root defaultValue="production"><Components.Select.Trigger aria-label="Environment"><Components.Select.Value placeholder="Choose environment" /></Components.Select.Trigger><Components.Select.Content><Components.Select.Item value="production">Production</Components.Select.Item><Components.Select.Item value="staging">Staging</Components.Select.Item><Components.Select.Item value="development">Development</Components.Select.Item></Components.Select.Content></Components.Select.Root>;
