...
describe("overlay components", () => {
  test("FR-CMP-006 AC-1 / FR-A11Y-002 AC-3: Dialog traps focus, closes on Escape and restores the trigger", async () => {
    ...
    expect(close.classList.contains("cdt-dialog__close")).toBe(true);
    expect(close.classList.contains("cdt-btn--secondary")).toBe(true);
    fireEvent.keyDown(getByRole("dialog"), { key: "Escape" });
    expect(document.querySelector("[role=dialog]")).toBeNull();
    await waitFor(() => expect(document.activeElement).toBe(trigger));
  });

  test("DEV-017: Dialog.Close preserves an asChild Button variant", () => {
    const { getByRole } = render(
    ...
    const close = getByRole("button", { name: "Confirm" });
    expect(close.classList.contains("cdt-dialog__close")).toBe(true);
    expect(close.classList.contains("cdt-btn--primary")).toBe(true);
    expect(close.classList.contains("cdt-btn--secondary")).toBe(false);
  });
  ...
  test("FR-CMP-006 AC-1 / AC-2: Drawer uses the Dialog Radix path and exposes its side class", () => {
    ...
    expect(
      getByRole("button", { name: "Close" }).classList.contains(
        "cdt-drawer__close",
      ),
    ).toBe(true);
  });
  ...
  test("FR-CMP-006 AC-3: Tooltip opens from keyboard focus and closes on Escape", async () => {
    ...
    expect((await waitFor(() => getByRole("tooltip"))).textContent).toContain(
      "Helpful text",
    );
  });
  ...
  test("FR-A11Y-005 AC-4: DropdownMenu keeps Radix menu roles and hides decorative icons", async () => {
    const { getByRole } = render(
      <DropdownMenu.Root defaultOpen>
        ...
        <DropdownMenu.Content>
          <DropdownMenu.Item icon="●">Item</DropdownMenu.Item>
        </DropdownMenu.Content>
      </DropdownMenu.Root>,
    );
    ...
    expect(
      item.querySelector(".cdt-menu__item-icon")?.getAttribute("aria-hidden"),
    ).toBe("true");
  });
  ...
  test("FR-CMP-006 AC-4: overlay content emits the tokenised public classes", () => {
    ...
    expect(
      document.querySelector(".cdt-overlay")?.classList.contains("cdt-overlay"),
    ).toBe(true);
  });
});
