...
- JOB-BUILD-004 산출물을 버전 식별자(배포 커밋의 SHA)가 붙은 불변 디렉터리에 업로드한다.
- 배포는 "현재 서비스 중" 별칭(pointer)을 새 버전 디렉터리로 전환하는 방식으로 수행한다. 파일을 덮어쓰는 방식은 채택하지 않는다: 덮어쓰기는 배포 도중 일부 방문자가 신·구 버전이 섞인 자산을 받는 상태를 만든다.
- 롤백은 별칭을 직전 버전 디렉터리로 되돌리는 것으로 수행하며, npm 패키지 롤백과 동일하게 10분 이내를 목표로 한다(NFR-004의 운영성 기준을 문서 사이트 배포에도 동일 적용한다).
- 직전 5개 버전 디렉터리를 보존해 롤백 대상이 항상 존재하게 한다. 6번째 이전 버전은 신규 배포 시 정리한다.
