...
0. **최초 namespace bootstrap만** npm 계정 2FA를 켜고 public organization `conductor-by-89soone`을 사용한다. 아직 존재하지 않는 패키지는 Trusted Publisher를 등록할 수 없으므로, 공개 저장소의 현재 `0.0.0` 패키지 3종을 `tokens → css → react` 순서로 `bootstrap` dist-tag에 대화형 게시한다. 장기 토큰이나 저장소 secret을 만들지 않는다. 각 패키지가 생성되면 npm 11.18.0의 `npm trust github <package> --file release.yml --repository 89sooner/design-system --allow-publish --yes`로 패키지별 신뢰 관계를 등록한다(웹 package Settings에서 같은 값을 입력해도 된다). 이후 모든 정식 릴리스는 아래 OIDC 절차만 사용하고 bootstrap 세션은 `npm logout`으로 폐기한다(CR-022, CR-027).
...
4. 배포 잡은 annotated tag 생성 전에 `github-actions[bot]`의 Git 작성자 이름과 이메일을 설정하고, `pnpm build`로 전체 빌드를 재실행한 뒤(로컬/PR 빌드 산출물을 신뢰하지 않는다) `NPM_CONFIG_PROVENANCE=true`로 `pnpm changeset publish`를 실행한다. 이 명령은 레지스트리에 없는 버전만 게시해 재실행에 안전하다(CR-015, CR-025).
5. `pnpm check:release-tags`는 세 패키지의 현재 manifest 버전에 대응하는 annotated tag가 존재하고 해당 커밋이 릴리스 HEAD의 조상인지 확인한다. 로컬 검증 뒤 태그를 push하고 `--remote origin`으로 원격 tag object가 로컬과 바이트 단위로 같은지 재검증한다. 하나라도 없거나 lightweight tag이면 배포 잡을 실패시킨다(CR-025).
...
- `main` push는 triggering SHA의 JOB-BUILD-004 산출물을 자동 배포한다. maintainer의 수동 트리거(`workflow_dispatch`)는 `ref` 입력으로 임의 커밋을 재배포하거나 롤백할 때만 사용한다(CR-032).
