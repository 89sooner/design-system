...
3. 변경 이력을 집계해 각 패키지의 semver 버전을 결정한다(`conductor_api_contracts.md` 6절). 변경 이력이 없는 패키지는 버전을 올리지 않는다(FR-DX-005 AC-3).
4. 수동 승인 게이트를 통과하면 OIDC 토큰으로 npm에 배포하고, git 태그와 `CHANGELOG` 항목을 생성한다.
