...
> 상태: review | 버전: v0.7 | 갱신일: 2026-09-13
...
컴포넌트 자체 polling/인증/원격 요청은 0이다. 이는 소비자의 기존 승인된 polling 정책을 없애라는 뜻이 아니다. 오래된 검색·선택·복사 응답은 소비자 generation/abort 계약과 컴포넌트 현재 맥락 검증으로 거부한다. CI는 실행 결과와 미실행 원인을 구별한다.
