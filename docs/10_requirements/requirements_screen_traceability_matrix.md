...
| 요구사항 | 화면 / 표면 | 구현 단위 | 검증 |
| --- | --- | --- | --- |
| FR-CMP-010 | W-060 / SFC-PKG | WP-030 | 검색·선택·미리보기·복귀, IME, 상태·커서 |
| FR-CMP-011 | W-061 / SFC-PKG | WP-031 | 그래프/목록 방향·근거, 결정성, 50/300노드 |
| FR-CMP-012 | W-062 / SFC-PKG | WP-032 | 단계 의미, 허용 행동, 부분/미확인 |
| FR-CMP-013 | W-060, W-061, W-062 / SFC-PKG / SFC-REL | WP-029 | Radix 키보드·포커스와 tarball 소비 |
...
| 화면 ID | 화면명 | 경로 | 요구사항 |
| --- | --- | --- | --- |
| W-060 | 검색 워크벤치 | `/examples/workbench` | FR-CMP-010, FR-CMP-013 |
| W-061 | 관계 탐색기 | `/examples/relations` | FR-CMP-011, FR-CMP-013 |
| W-062 | 수집 운영 상태 | `/examples/operations` | FR-CMP-012, FR-CMP-013 |

W-040 가이드 화면은 유지하며 실제 조합은 독립 경로로 연결한다. 이는 화면을 URL로 직접 재현하기 위한 구현 선택이다.
