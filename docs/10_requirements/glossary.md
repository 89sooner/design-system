...
| 한국어 | English/code | 의미 |
| --- | --- | --- |
| 검색 워크벤치 | Search workbench | 조건·로드된 결과·미리보기·복귀를 조합한 표현 |
| 관계 탐색기 | Relation explorer | 안정 ID 노드와 방향 간선의 읽기 전용 그래프/목록 |
| 근거 | Evidence | 소비자가 제공한 관계 판단의 텍스트 근거; 패키지가 추론하지 않음 |
| 미해결 / 모호함 / 해제 | unresolved / ambiguous / detached | 대상 해석 불가 / 여러 해석 / 해제된 관계; 실제 0건과 다름 |
| 부분 결과 | Partial result | 일부 데이터가 누락되거나 미확인인 결과 |
| 커서 | Cursor | 이어 보기에 소비자가 관리하는 불투명 값; 공유 URL에 노출하지 않음 |
| 운영 상태 | Pipeline status | 수신·내구 저장·처리·색인 각 단계의 확인된 상태 |
