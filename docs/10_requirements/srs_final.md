...
- OD-003의 범위 조건은 CR-041 요청으로 해소했다. 새 FR의 정식 baseline 승인은 별도다.
- 이 문서의 `baseline` 승격은 사용자 승인으로만 이루어진다.

## 17. CR-041 확장 요구 (review)

기존 v1.6 baseline 이력은 유지하고 이번 확장만 재검토한다. 아래 블록은 로컬 구현 근거이며 정식 baseline 승인 기록이 아니다.
