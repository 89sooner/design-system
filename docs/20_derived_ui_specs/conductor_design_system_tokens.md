...
기존 semantic surface/text/border/focus와 density를 먼저 사용한다. 업무 영역의 평탄한 표면은 명시적 클래스/variant로 조합하며 기존 Card 기본값을 무조건 바꾸지 않는다. 필요 신규 토큰은 원본에서 추가하고 두 테마 키를 동일하게 유지한다. 생성 TypeScript 토큰을 편집하지 않는다.
