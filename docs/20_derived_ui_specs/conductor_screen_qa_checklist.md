...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| QA-001 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-002 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-003 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-004 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-005 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-006 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-007 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |
| QA-008 | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] | [x] |

실행 근거: `apps/docs/e2e/screens.spec.ts`가 12개 화면을 다크/라이트 × 560/800/1080에서 순회해 문서 폭·주요 요소 clipping·표/코드 자체 overflow를 검사하고, 두 테마에서 화면별 DOM 순서와 실제 Tab 순서 및 `:focus-visible` box-shadow를 대조한다. 2026-07-15 실행 결과 레이아웃 6/6, 포커스 2/2 통과.

## 2. 화면별 QA

### W-001 Overview

- [x] QA-009 `data-cdt-theme` 속성이 없을 때, OS가 `prefers-color-scheme: dark`이면 다크 팔레트가, 그 외에는 라이트 팔레트가 적용된다 (FR-THM-003 AC-2)
- [x] QA-010 `data-cdt-theme="light"`이고 OS가 다크일 때 라이트 팔레트가 적용된다 (FR-THM-003 AC-1)
- [x] QA-011 테마 전환 시 컴포넌트가 재마운트되지 않고 CSS 커스텀 프로퍼티 값만 바뀐다 (FR-THM-003 AC-4)
...
- [x] QA-014 문서 사이트가 `@conductor-by-89soone/react`와 `@conductor-by-89soone/css`를 소비자로서 설치해 사용하며, 소스 상대경로 import가 0건이다 (FR-DOC-001 AC-1)

### W-002 Getting Started

- [x] QA-015 설치 절차가 패키지 설치 → CSS import → `data-cdt-theme` 지정 → `Button` 렌더 순서로 기술되어 있다 (SCN-001 기본 흐름)
- [x] QA-016 `@conductor-by-89soone/css`를 import하지 않고 컴포넌트를 렌더하면 개발 빌드 콘솔 경고가 1회 출력됨을 안내 문구로 명시한다 (SCN-001 예외 흐름)
- [x] QA-017 레이어 선언 순서가 산출물 최상단 한 줄에 고정되어 있음을 코드 예시로 보여준다 (FR-CSS-001 AC-4)
- [x] QA-018 서버 렌더링 시 브라우저 전역 접근이 발생하지 않는다는 설명과, 첫 페인트 테마 결정을 위한 인라인 스니펫 예시를 제공한다 (FR-DX-004, FR-THM-003 예외처리)

### W-010 Foundations · Color

- [x] QA-019 색상 값이 `tokens.json`에서 읽히며 화면에 하드코딩된 토큰 값이 0건이다 (FR-DOC-002 AC-1)
- [x] QA-020 토큰 소스에 색상 토큰을 추가하고 재빌드하면 화면에 자동으로 나타난다 (FR-DOC-002 AC-2)
- [x] QA-021 각 토큰 행이 토큰 키, 계층, 현재 테마 값, 용도 설명을 표시한다 (FR-DOC-002 AC-3)
- [x] QA-022 상태 7종·심각도 4종·미터 3종 토큰이 두 테마 모두에 표시된다 (FR-TOK-005 AC-4)
- [x] QA-023 primitive 토큰이 화면에 노출되지 않는다 (FR-TOK-002 AC-5)

### W-011 Foundations · Typography

- [x] QA-024 타이포 값이 `tokens.json`에서 읽히며 화면에 하드코딩된 토큰 값이 0건이다 (FR-DOC-002 AC-1)
- [x] QA-025 `font.size` 7단계(2xs~xl)와 대응하는 `font.lineHeight`가 표시된다 (FR-TOK-007 AC-1, AC-2)
- [x] QA-026 제목 예시가 `font.size.xl` 이상과 `clamp()` 기반 값을 사용한다 (FR-TOK-007 AC-4)

### W-012 Foundations · Spacing & Layout

- [x] QA-027 레이아웃 값이 `tokens.json`에서 읽히며 화면에 하드코딩된 토큰 값이 0건이다 (FR-DOC-002 AC-1)
- [x] QA-028 `breakpoint` 3단계(sm/md/lg) 값이 표시된다 (FR-TOK-009 AC-1)
- [x] QA-029 `cdt-split-layout` 예시가 뷰포트 800px 미만에서 단일 컬럼으로 전환된다 (FR-CSS-003 AC-2)
- [x] QA-030 `cdt-card-grid` 예시가 뷰포트 560px 미만에서 단일 컬럼으로 전환된다 (FR-CSS-003 AC-3)

### W-013 Foundations · Radius & Elevation

- [x] QA-031 반경/고도 값이 `tokens.json`에서 읽히며 화면에 하드코딩된 토큰 값이 0건이다 (FR-DOC-002 AC-1)
- [x] QA-032 `elevation.*` 토큰의 그림자 alpha 값이 다크 테마와 라이트 테마에서 다른 값으로 표시된다 (FR-THM-002 AC-4)

### W-014 Foundations · Motion

- [x] QA-033 모션 값이 `tokens.json`에서 읽히며 화면에 하드코딩된 토큰 값이 0건이다 (FR-DOC-002 AC-1)
- [x] QA-034 `prefers-reduced-motion: reduce` 상태에서 전환/애니메이션 지속 시간이 0s로 표시된다 (FR-CSS-005 AC-1)
- [x] QA-035 감소 모드에서도 hover/focus/selected 상태 변화의 최종 시각 결과가 감소 모드가 아닐 때와 동일하다 (FR-CSS-005 AC-2)

### W-020 Components Index

- [x] QA-036 공개 진입점에 export된 모든 컴포넌트가 카탈로그에 화면으로 존재한다 (FR-DOC-003 AC-5)
- [x] QA-037 각 카탈로그 항목이 실제 DOM으로 마운트되어 렌더된다(스크린샷 이미지 0건) (FR-DOC-003 AC-1)
- [x] QA-038 `cdt-` 접두사 클래스만으로 React 없이도 `Button variant="primary"`와 동일한 계산된 스타일이 재현됨을 코드 예시로 확인한다 (FR-CSS-004 AC-3)

### W-021 Component Detail

- [x] QA-039 선택한 컴포넌트의 모든 `variant`와 `tone` 조합이 프리뷰에 렌더된다 (FR-DOC-003 AC-3)
- [x] QA-040 props 표가 `@conductor-by-89soone/react` 타입 정의에서 생성되며 수동 작성 행이 0건이다 (FR-DOC-003 AC-2)
- [x] QA-041 프리뷰가 현재 선택된 테마를 따른다 (FR-DOC-003 AC-4)
- [x] QA-042 코드 스니펫 복사 버튼 클릭 시 2초 이내 `복사됨` 상태가 표시되고 이후 원래 상태로 복귀한다 (FR-DOC-006 AC-1)
- [x] QA-043 복사 완료가 `aria-live="polite"` 영역으로 알려진다 (FR-DOC-006 AC-2)
- [x] QA-044 선택한 컴포넌트에 대해 ref 전달, className 병합, data-*/aria-* 통과, 네이티브 props 확장이 검증된다 — 개별 판정은 3절 컴포넌트 QA의 해당 행을 인용한다 (FR-CMP-001 AC-1~AC-4)

### W-030 Tokens Reference

- [x] QA-045 토큰 키 문자열로 필터하면 일치하는 행만 남는다 (FR-DOC-004 AC-1)
- [x] QA-046 색상 토큰 행이 다크 값과 라이트 값을 나란히 표시한다 (FR-DOC-004 AC-2)
- [x] QA-047 대비 검사 대상 토큰 쌍에 대비율 수치와 pass/fail 판정이 표시된다 (FR-DOC-004 AC-3)
- [x] QA-048 대비 검사 제외 토큰에 `장식 전용` 표식과 제외 사유가 표시된다 (FR-DOC-004 AC-4)
- [x] QA-049 테마 토글이 `role="switch"` 또는 `aria-pressed`를 노출하고 키보드로 조작 가능하다 (FR-DOC-005 AC-5)

### W-040 Patterns

- [x] QA-050 각 규칙이 권장 예와 금지 예를 실제 렌더된 컴포넌트로 나란히 보여준다 (FR-DOC-007 AC-1)
- [x] QA-051 금지 예에 금지 사유가 문장으로 기재된다 (FR-DOC-007 AC-2)
- [x] QA-052 상태 7종과 심각도 4종 각각의 사용 시점이 기술된다 (FR-DOC-007 AC-3)
- [x] QA-053 `Dialog`와 `Drawer`의 선택 기준이 기술된다 (FR-DOC-007 AC-4)

### W-050 Accessibility

- [x] QA-054 `pnpm check:contrast` 결과에서 두 테마 전체 대비 미달이 0건으로 보고된다 (FR-A11Y-004 AC-1)
- [x] QA-055 axe-core 허용 목록이 규칙 ID와 사유와 함께 노출된다 (FR-A11Y-005 예외처리)
- [x] QA-056 포커스 링이 두 테마 모두에서 배경 대비 3:1 이상임을 명시한다 (FR-A11Y-001 AC-3)
...
† FR-CMP-009는 Should 우선순위이며 OD-004에 조건부다. OD-004가 "패키지에 포함"으로 해소되는 경우에만 QA-165 ~ QA-176을 `@conductor-by-89soone/react`의 공개 컴포넌트로 실행한다. "문서 사이트 내부 컴포넌트로 강등"으로 해소되면 이 3개 행은 공개 컴포넌트 QA 대상에서 제외하고, 문서 사이트 자체 구현 코드에 대한 QA로 이관한다(SRS FR-CMP-009 예외처리).

## 4. 접근성 QA

- [x] QA-177 WCAG 2.1 AA 1.4.1(색상 외 정보 전달) 충족을 확인한다 (FR-A11Y-003)
...
- [x] QA-183 컴포넌트 전수를 그레이스케일로 렌더한 스냅샷에서 상태 7종의 구분이 유지된다 (FR-A11Y-003 AC-4) — 공개 30개 전수와 상태 7종·심각도 4종 기준 이미지 3장, 고정 Chromium 비교 통과
...
- [x] QA-186 다크/라이트 테마의 semantic 토큰 키 집합 대칭 차집합이 공집합이다 (FR-QA-001 AC-1)
- [x] QA-187 한 테마에만 존재하는 component 토큰이 0건이다 (FR-QA-001 AC-2)
- [x] QA-188 토큰 계약 검사가 `pnpm test`와 CI 모두에서 실행된다 (FR-QA-001 AC-3)
...
이 절은 이 제품이 런타임 인증/권한/서버 없이 정적 산출물만 배포한다는 사실에서 도출되는 고유 상태를 다룬다(`conductor_screen_state_matrix.md` 4절과 동일한 정의를 인용한다).

- [x] QA-193 **deprecated (CR-020)** — SRS FR-DOC-002에 없는 런타임 폴백 요구를 파생 문서가 추가한 오류였다. `tokens.json`은 정적 사이트의 필수 빌드 입력이며 부재는 화면 상태가 아니라 빌드 실패다.
- [x] QA-194 대비 검사 결과 파일이 없을 때 W-030, W-050의 대비율 관련 영역이 `측정되지 않음`으로 표시되고 화면 상단에 경고 배너가 노출된다 (FR-DOC-004 예외처리)
- [x] QA-195 컴포넌트 프리뷰 렌더 중 예외가 발생해도 해당 프리뷰 영역만 오류 경계로 격리되고 나머지 화면은 계속 렌더된다 (FR-DOC-003 예외처리)
- [x] QA-196 Clipboard API가 없는 환경에서 코드 블록 텍스트가 선택 가능한 상태로 유지되고 복사 버튼이 `disabled`로 렌더된다 (FR-DOC-006 AC-3)
- [x] QA-197 클립보드 쓰기가 거부되면 복사 버튼에 `복사할 수 없음` 상태가 표시되고 오류 배너는 노출되지 않는다 (FR-DOC-006 예외처리)
- [x] QA-198 `localStorage` 접근이 차단된 환경에서 예외가 삼켜지고 `prefers-color-scheme`으로 대체되며 화면 렌더가 막히지 않는다 (FR-DOC-005 예외처리)
- [x] QA-199 `prefers-reduced-motion: reduce` 활성 시 `Spinner`와 `ProgressRing`이 애니메이션 대신 정적 진행률 텍스트를 노출한다 (FR-CSS-005 예외처리)
- [x] QA-200 화면 어디에도 권한 없음 안내나 인증 만료 안내가 존재하지 않는다 — 존재하면 결함으로 기록한다(해당 없음: 런타임 인증/서버 부재, SRS 6절 권한 경계)
...
FR-CMP-010~013: 360/768/1280/1536px, dark/light, density 위험 기반 매트릭스; 한영 긴 제목/경로·40자 식별자, IME·200% 확대·reduced motion·키보드·overlay 충돌을 검사한다. axe와 수동 보조기술 검증을 구별하고 NOT RUN을 남긴다. 그래프 50/300노드, 결과 0/1/100/1000의 입력·측정법·DOM 수를 기록한다.
