...
| CR-033 | 2026-08-06 | design + implementation | 분석 리포트 적용(APPLY.md D-01 ~ D-06) + `DEV-026` | 문서 사이트의 화면당 URL을 하나로 확정한다. W-030·W-040이 두 경로에서 렌더되고 사이드 내비와 IA가 서로 다른 쪽을 정규로 삼던 상태를 내비 쪽(`/tokens/reference`, `/guidelines`)으로 통일하고, 옛 경로는 쿼리스트링을 보존하는 `replace` 리다이렉트로 남긴다. 함께 `lint:tokens` 대상에 `apps/docs/src`를 추가하고 `rem-literal` 규칙을 신설해 문서 사이트도 토큰 소스를 통과하게 만든다(생성물 디렉터리는 스캔에서 제외한다). 화면 ID·FR 매핑·공개 API·범위는 바꾸지 않는다 | DEV-026, W-030, W-040, FR-TOK-001, FR-DOC-001, FR-DOC-004, FR-DOC-007, WP-006, WP-021, WP-022, WP-028 | `conductor_product_ia.md`, `conductor_ui_component_spec.md`, docs 앱 라우팅·테마·포커스, 토큰 린트 규칙, 배송 원장 | closed |
| CR-034 | 2026-08-06 | design | 분석 리포트 적용(APPLY.md G-01, G-02) | 토큰 세트에 없던 두 스케일을 추가한다. (1) 선 두께 `border.width.hairline/emphasis/rail`과 그 기반 primitive `stroke.1/2/3`. 스타일시트의 모든 경계가 리터럴 `1px`이었고 그때마다 “토큰에 선 두께가 없다”는 허용 주석이 짝지어 붙어 있었다 — 결정으로 비운 자리가 아니라 빠뜨린 자리다. `table.headerBorderWidth`·`badge.marker.dotRingWidth`의 `2px` 리터럴도 별칭으로 바꾼다. (2) `page.sectionHeadingSize`·`page.subHeadingSize`와 각 줄 높이. `h2`·`h3`가 사용자 에이전트 기본값으로 떨어져 `h1` 아래 위계가 없었다. `page.headingSize`와 같이 `font.size.xl`에서 파생하므로 FR-TOK-007 AC-1의 타입 스케일 7단은 그대로다. 색·대비 쌍·공개 React API는 바꾸지 않는다 | FR-TOK-002, FR-TOK-007, FR-THM-001, FR-CSS-002, FR-CSS-004, WP-002, WP-005, WP-008 | `conductor_design_system_tokens.md`, 토큰 소스(primitives·scales·components), `packages/css/src/base.css`·`components.css`, `apps/docs/src/docs.css`, 배송 원장 | closed |
...
- [x] Release run 29585593807 — version PR #4가 Changeset을 정상 소비한 merge SHA `15024d2`에서 publish 전 version 잡만 실패했고 npm publish job은 skip됐음을 확인
...
**요구사항 변경이 없다.** 게이트의 위치와 실패 처리를 고친다.

- [x] `scripts/check-release-tags.mjs` — `--snapshot`이 이번 실행이 발행할 패키지의 낡은 태그를 그 자리에서 거부한다. `registryHasVersion`은 404만 `false`로 답하고 다른 실패에는 종료 1로 중단한다
- [x] `.github/workflows/release.yml` — 단계 이름을 `reject stale release tags and snapshot the registry`로 바꾸고, 이 단계가 발행 앞에 있어야 하는 이유를 주석에 적었다
...
- [x] **정정 (DEV-044)**: 위 두 문서가 처음에는 "게시하지 않은 패키지의 태그는 판정 대상이 아니다"라고 적었는데 코드보다 넓은 말이었다. **게시 뒤 모드**(`--published-before`)는 건너뛴 패키지에도 존재·annotated 여부·HEAD 도달성·원격 일치를 적용하며 면제되는 것은 "태그 대상 == 릴리스 HEAD" 하나뿐이다. **게시 전 모드**(`--snapshot`)는 레지스트리에 이미 있는 버전의 태그를 아예 보지 않는다 — 두 모드를 구분해 적었다

## 6. 미해소 오픈 결정

| 결정 ID | 차단 대상 | baseline 차단 여부 | 담당 | 기한 |
| --- | --- | --- | --- | --- |
| OD-003 | CR-041로 범위 조건 해소 (FR-CMP-010) | 새 baseline 승인 별도 | Product | 2026-09-13 |
...
## CR-041 업무 탐색 조합 확장 (2026-09-13)

| 항목 | 기록 |
| --- | --- |
| 유형 / 상태 | scope + implementation / review |
| 변경 근거 | 사용자의 2026-09-13 실행 지시: 검색·선택·복귀, 관계 방향·근거, 수집·처리·색인의 세 흐름을 코드·토큰·문서·합성 화면·패키지 소비로 검증 |
| 사용자 명시 범위 | F-CMP-010 조건부 해소, 읽기 전용 관계 시각화 예외 포함, 공통 상호작용과 운영 상태 표현. 서버·인증·추론·운영 쓰기는 제외 |
| 추가 요구 | FR-CMP-010, FR-CMP-011, FR-CMP-012, FR-CMP-013 |
| 구현 단위 | WP-029, WP-030, WP-031, WP-032 (REL-004 확장 검증) |
| 승인 구분 | 사용자가 로컬 구현 범위 확장을 명시했다. 정식 baseline 재승인은 받지 않았으므로 변경 SRS는 review다. 기존 v1.6 승인 이력은 보존 |
| Agent 추가 결정 | ADR-011의 ADEC 로그에 별도 기록. 사용자 명시 기능을 Agent 발명으로 분류하지 않는다 |
| 검증 | 종료 validator --report exit 0, FR 53/53·화면 15/15. 코드·접근성·브라우저·tarball·격리 소비자 증거는 구현 원장 CR-041 종료 기록에 추가. strict는 사용자 baseline 승인 전용 gate로 exit 1 |
| cascade | SRS → PRD → 용어 → 매트릭스 → IA/와이어/흐름/상태/컴포넌트/토큰/QA → 아키텍처 → 전달 문서 → 실행 브리프 순서로 반영 |

OD-003의 필터/칩 범위 조건은 이번 사용자 요청으로 해소한다. 별도 baseline 승인 대기는 구현 중단 사유가 아니다. CR-031·CR-032의 기존 미실행 외부 배포 증거는 이번 로컬 작업에서 닫지 않는다.

CR-041 구현 cascade와 로컬 검증은 2026-09-14에 종료했다. 상태는 review를 유지한다. 이 기록은 npm 게시, 원격 push, baseline 승인 또는 실제 소비자 운영 연동 완료를 뜻하지 않는다.
