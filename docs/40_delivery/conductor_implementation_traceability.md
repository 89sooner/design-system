...
> 상태: review | 버전: v0.13 | 갱신일: 2026-07-17
...
| WP-027 | Changesets와 npm 배포 워크플로 | REL-004 | done | `4411aa1` | version PR #2와 OIDC Release run 29569125471로 0.1.0 3종·SLSA provenance v1 게시. actual rollback 323.8초. 첫 publish에서 Changesets의 annotated tag 실패가 무음 통과한 DEV-018은 bot Git identity와 `check:release-tags`의 로컬/원격 tag object 게이트로 해소(CR-025). `core.autocrlf=true`에서 유효한 CRLF Changeset을 거부한 DEV-022는 입력 줄바꿈 정규화로 해소(CR-029) | 2026-07-17 |
...
| FR-DX-005 | `.changeset/config.json`, `scripts/check-changesets.mjs`, `scripts/check-api.mjs`, `scripts/check-release-tags.mjs`, `scripts/release-rollback.mjs`, `.github/workflows/release.yml` | `check:changesets`·`check:api` 음성 픽스처, `changeset version`, 0.1.0 OIDC publish·provenance, annotated tag local/remote 검증, npm rollback 실증 | WP-027 | 검증됨 (AC-1 드리프트 exit 1 + major 규약, AC-2 version PR #2의 CHANGELOG Refs와 LF/CRLF 동일 파싱, AC-3 react-only bump 실측, AC-4 마이그레이션 노트 강제, 예외 처리 `changeset status --since`. Release run 29569125471과 323.8초 실제 롤백으로 외부 경로까지 확인) |
