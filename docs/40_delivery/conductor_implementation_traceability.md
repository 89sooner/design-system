...
| WP-011 | `@conductor/react` 골격과 공통 계약 | REL-002 | done | `a46af8d` | React 18/19·react-dom·lucide peer 계약과 `sideEffects: false` 선언. `cx`, `PolymorphicProps`, 공유 ref/className/data/aria/native-props 계약 스위트, public component registry·빌드 전 누락 테스트 검사, SSR `renderToString` 하네스 추가. React 패키지 18/18 테스트, build·typecheck 통과 | 2026-07-11 |
...
| WP-016 | 폼 컴포넌트군 | REL-002 | done | `ce00050` | `pnpm --filter @conductor/react test -- form` 121/121, CSS 72/72, build/typecheck/test/lint:deps/lint:tokens/check:contrast 통과. `pnpm lint`는 이번 WP와 무관한 기존 `testing/contract.test.tsx` ESLint 오류 3건으로 실패 | 2026-07-11 |
...
| WP-024 | 접근성 검사 CI 잡 | REL-003 | done | `1bdfe8b` | Vitest browser + Playwright Chromium에서 공개 30개·49상태×2테마 axe, 30개 키보드 경로, overlay Escape/이탈, focus-visible, cascade를 검증. 정상 134 passed/1 fixture skipped, critical `button-name` fixture는 exit 1. axe JSON/실패 screenshot CI 아티팩트 30일 보존. CI에서는 브라우저 provisioning(`playwright install`)이 선행되어야 한다 — pnpm 10이 lifecycle script를 차단해 설치만으로는 브라우저가 없다(WP-028에서 발견·수정) | 2026-07-12 |
...
| WP-027 | Changesets와 npm 배포 워크플로 | REL-004 | done | `4411aa1` | Changesets 2(linked @conductor 3종)와 규약 검사(`check:changesets`: Refs 줄·major 마이그레이션 노트), api-extractor 7 리포트 3종(`check:api`: 드리프트·`any` 게이트), 시크릿 스캔 7패턴, version PR/OIDC publish 분리 release.yml, dist-tag 롤백 스크립트를 구현. `changeset version` 실험에서 react만 0.1.0(AC-3)·CHANGELOG Refs 포함(AC-2), 드리프트·규약·시크릿 음성 픽스처 각 exit 1, audit high 0건, 3패키지 dry-run 배포 성공 | 2026-07-13 |
...
| FR-DX-002 | 각 패키지 `package.json`의 `exports`·`types`, `tsup` DTS 산출, `packages/react/src/types.ts`, `scripts/check-api.mjs` | `packages/*/src/index.test.ts`, `packages/react/src/index.test.ts`, `packages/tokens/src/build/emit-artifacts.test.ts`, `packages/*/etc/*.api.md`, 소비자 스모크(§4) | WP-001, WP-004, WP-011, WP-018, WP-027, WP-028 | 검증됨 (AC-1 `types`/`exports.types` 선언, AC-2 `any` 0건을 `check:api`가 상시 강제, AC-3은 workspace 밖 신규 앱이 `pnpm pack` tarball을 설치해 `tsc --noEmit` 0 오류로 통과(§5), AC-4 내부 타입 누출을 SRS가 지정한 공개 API 추출 리포트로 확인) |
...
| FR-DX-005 | `.changeset/config.json`, `scripts/check-changesets.mjs`, `scripts/check-api.mjs`, `scripts/release-rollback.mjs`, `.github/workflows/release.yml` | `check:changesets`·`check:api` 음성 픽스처, `changeset version` 실험, dry-run 배포 | WP-027 | 구현됨 (AC-1 드리프트 exit 1 + major 규약, AC-2 CHANGELOG Refs, AC-3 react-only bump 실측, AC-4 마이그레이션 노트 강제, 예외 처리 `changeset status --since`. 실 레지스트리 게시·롤백 검증은 첫 배포에서 수행 — §5) |
...
| 소비자 스모크가 CI에 자동화되지 않았고, 패키지를 `pnpm pack` tarball로 소비한다 | M-5(신규 앱 적용 명령 3개 이하)와 FR-DX-002 AC-3(소비자 `tsc --noEmit`)을 workspace 밖 신규 Vite React 앱에서 1회 실측했다(문서의 3개 명령 그대로, 타입 오류 0건, 프로덕션 빌드 성공). 아직 게시 전이라 `@conductor/*`는 레지스트리가 아니라 tarball에서 해석되며, 이 절차는 CI 잡이 아니다 | M-5, FR-DX-002 AC-3, QA-205, WP-028 | 첫 배포 후 레지스트리 설치로 같은 절차를 반복하고, 필요하면 CI 잡으로 승격한다 |
| GitHub Pages 실 배포·롤백 리허설이 첫 배포 전까지 미실행이다 | 배포 워크플로(수동 승인, `ref` 지정 롤백, Pages 원자적 스냅샷)가 워크플로 inspection과 로컬 빌드·게이트 재현으로만 검증됐다. 실제 Pages 환경의 배포 시간과 롤백 10분 예산(NFR-004)은 첫 배포에서 실측한다 | FR-DOC-001, NFR-004, WP-028 | 첫 배포 직후 롤백 리허설을 실측해 이 행을 닫는다 |
| npm 실 레지스트리 검증이 첫 배포 전까지 미실행이다 | OIDC 신뢰 게시와 provenance, `@conductor` 스코프 확보(THR-001), 실 레지스트리 롤백 리허설이 워크플로 inspection·dry-run 배포·dry-run 롤백으로만 검증됐다 | FR-DX-005, NFR-002, NFR-004, WP-027 | 첫 배포에서 신뢰 게시와 provenance를 확인하고, 직후 롤백 리허설을 실측(10분 예산)해 이 행을 닫는다 |
