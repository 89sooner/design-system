...
| FR-A11Y-002 | `packages/react/src/data.tsx`, `src/overlay.tsx`, `src/shell.tsx` | `packages/react/src/testing/data.test.tsx`, `overlay.test.tsx`, `shell.test.tsx`, `apps/docs/e2e/shell.spec.ts` | WP-014, WP-015, WP-023 | 부분 (Dialog/Drawer와 AppShell off-canvas Escape, Timeline 대화형 단계, skip-link focus 이동을 검증. 전 컴포넌트 키보드 경로 전수 검증은 WP-024에서 완성.) |
