...
> 상태: review | 버전: v0.10 | 갱신일: 2026-07-17
...
| WP-011 | `@conductor-by-89soone/react` 골격과 공통 계약 | REL-002 | done | `a46af8d` | React 18/19·react-dom·lucide peer 계약과 `sideEffects: false` 선언. `cx`, `PolymorphicProps`, 공유 ref/className/data/aria/native-props 계약 스위트, public component registry·빌드 전 누락 테스트 검사, SSR `renderToString` 하네스 추가. React 패키지 18/18 테스트, build·typecheck 통과 | 2026-07-11 |
...
| npm 실 레지스트리 검증이 첫 배포 전까지 미실행이다 | GitHub admin은 확인했지만 npm CLI는 미로그인이고 `@conductor-by-89soone/*` 3종은 registry 404다. npm org `conductor` 생성·2FA·`bootstrap` dist-tag 최초 게시·패키지별 Trusted Publisher 등록이 필요하다. OIDC/provenance와 실 레지스트리 롤백은 아직 실행되지 않았다 | FR-DX-005, NFR-002, NFR-004, WP-027, CR-022 | 저장소 public 전환과 namespace bootstrap 후 정식 릴리스에서 OIDC/provenance를 확인하고, 직후 롤백 리허설을 실측(10분 예산)해 이 행을 닫는다 |
