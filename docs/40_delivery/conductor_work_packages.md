...
| WP-028 | 문서 사이트 정적 배포 | REL-004 | WP-022, WP-027 | - | done |
...
- 선행 WP: WP-022, WP-027
- 구현 범위: 정적 빌드 산출(라우트 코드 분할 + 랜딩 프리렌더), 배포 워크플로, Lighthouse CI 게이트, 배포 롤백 절차
...
## WP-029 공통 상호작용과 tarball 경계

- 릴리스: REL-004 (CR-041 로컬 확장)
- 관련 요구사항: FR-CMP-013
- 관련 화면: W-060, W-061, W-062
- 선행 WP: WP-028
- 구현 범위: Radix 상호작용·AppShell 포커스 재현, 배포 경계, React18/19/Next packed 소비
- 제외: 운영 API 쓰기·원격 게시·서버 정책·소비자 원본 작업 트리 변경
- 완료 기준(DoD): 해당 FR의 모든 AC를 코드/타입/문서/합성 브라우저 증거로 연결하고 실행·미실행을 구분한다.
- 검증 방법: 단위/접근성/브라우저·tarball fixture·기존 루트 게이트
- 기록: 구현 원장 WP-029, FR-CMP-013 매핑과 CR-041 증거

## WP-030 검색 결과 미리보기 복귀

- 릴리스: REL-004 (CR-041 로컬 확장)
- 관련 요구사항: FR-CMP-010
- 관련 화면: W-060
- 선행 WP: WP-029
- 구현 범위: IME·필터·semantic Table·안정 ID·커서·경로·미리보기 복귀 합성 화면
- 제외: 운영 API 쓰기·원격 게시·서버 정책·소비자 원본 작업 트리 변경
- 완료 기준(DoD): 해당 FR의 모든 AC를 코드/타입/문서/합성 브라우저 증거로 연결하고 실행·미실행을 구분한다.
- 검증 방법: 단위/접근성/브라우저·tarball fixture·기존 루트 게이트
- 기록: 구현 원장 WP-030, FR-CMP-010 매핑과 CR-041 증거
...
- 관련 화면: W-061
- 선행 WP: WP-030
- 구현 범위: 선택·방향·범례·근거·목록·부분·결정적 레이아웃과 50/300노드 측정
- 제외: 운영 API 쓰기·원격 게시·서버 정책·소비자 원본 작업 트리 변경
- 완료 기준(DoD): 해당 FR의 모든 AC를 코드/타입/문서/합성 브라우저 증거로 연결하고 실행·미실행을 구분한다.
- 검증 방법: 단위/접근성/브라우저·tarball fixture·기존 루트 게이트
- 기록: 구현 원장 WP-031, FR-CMP-011 매핑과 CR-041 증거
...
- 관련 화면: W-062
- 선행 WP: WP-031
- 구현 범위: 수신/저장/처리/색인과 허용 행동, 세 화면 테마/폭·패키지 회귀·마이그레이션
- 제외: 운영 API 쓰기·원격 게시·서버 정책·소비자 원본 작업 트리 변경
- 완료 기준(DoD): 해당 FR의 모든 AC를 코드/타입/문서/합성 브라우저 증거로 연결하고 실행·미실행을 구분한다.
- 검증 방법: 단위/접근성/브라우저·tarball fixture·기존 루트 게이트
- 기록: 구현 원장 WP-032, FR-CMP-012 매핑과 CR-041 증거
